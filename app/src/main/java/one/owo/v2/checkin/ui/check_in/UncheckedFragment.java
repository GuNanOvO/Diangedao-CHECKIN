...
public class UncheckedFragment extends Fragment {
    private RecyclerView uncheckedInNameRecyclerView;
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        checkInViewModel.getPersonList().observe(getViewLifecycleOwner(), data -> {
            System.out.println("uncheckedin observe被调用");
            personList = data;
            updateNames();
        });
        startCheckIn(view);
...
//        });
    ...
    private void startCheckIn(View view) {
        ...
        updateNames();
        // 初始化 RecyclerView
        uncheckedInNameRecyclerView = view.findViewById(R.id.uncheckedInNameRecyclerView);
        LinearLayoutManager layoutManager = new LinearLayoutManager(requireContext());
        uncheckedInNameRecyclerView.setLayoutManager(layoutManager);

        // 占位数据
        //names = new ArrayList<>();
        nameAdapter = new CheckInListNameAdapter(names, checkInViewModel);
        uncheckedInNameRecyclerView.setAdapter(nameAdapter);

        uncheckedInNameRecyclerView.post(() -> {
            int fontSize = nameAdapter.calculateOptimalFontSize(uncheckedInNameRecyclerView);
